...
bngl_files_folder = script_folder + "Files for bngl/";

%% using the updated model with new parameters governing CaMKII autophosphorylation

proj = sbioloadproject(script_folder+ "model_Nair_2018.sbproj");
...
save_stimulation(convertStringsToChars(bngl_files_folder+"stim_noDA.tsv"),t_eq+t_noDA,x_noDA(:,2:3),{'setParam','DA_Spine_parameter0'; 'setParam','Ca_Spine_parameter0'},name_rnf)
save_stimulation(convertStringsToChars(bngl_files_folder+"stim_noDA_simple.tsv"),t_eq+t_noDA,x_noDA(:,2:3)*fconv,{'setConc','DA()@Spine'; 'setConc','Ca()@Spine'; })

do_add=0;
for j=1:30
update_concentrations(convertStringsToChars(bngl_files_folder+"stim_noDA_complex.tsv"),(j-1)*t_noDA(end),initial_concentrations,[],do_add)
do_add=1;
save_stimulation(convertStringsToChars(bngl_files_folder+"stim_noDA_complex.tsv"),(j-1)*t_noDA(end)+t_noDA,x_noDA(:,2:3)*fconv,{'setConc','DA()@Spine'; 'setConc','Ca()@Spine'; },[],do_add);
end
...
save_stimulation(convertStringsToChars(bngl_files_folder+"stim_DA.tsv"),t_eq+t_DA,x_DA(:,2:3),{'setParam','DA_Spine_parameter0'; 'setParam','Ca_Spine_parameter0'},name_rnf)
save_stimulation(convertStringsToChars(bngl_files_folder+"stim_DA_simple.tsv"),t_eq+t_DA,x_DA(:,2:3)*fconv,{'setConc','DA()@Spine'; 'setConc','Ca()@Spine'; })
do_add=0;
for j=1:30 
update_concentrations(convertStringsToChars(bngl_files_folder+"stim_DA_complex.tsv"),(j-1)*t_DA(end),initial_concentrations,[],do_add)
do_add=1;
save_stimulation(convertStringsToChars(bngl_files_folder+"stim_DA_complex.tsv"),(j-1)*t_DA(end)+t_DA,x_DA(:,2:3)*fconv,{'setConc','DA()@Spine'; 'setConc','Ca()@Spine'; },[],do_add);
end
...
dn=1;
prt=1;
fnm=convertStringsToChars(bngl_files_folder+"stim_complex_"+num2str(prt)+".tsv");
for n = 1:length(DA)
