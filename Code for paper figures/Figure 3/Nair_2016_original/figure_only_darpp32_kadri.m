...
for n = 1:1
    ...
    calcium{1} = simWithoutDA{n}.Data{1}(2,:);
    time{1} = simWithoutDA{n}.Time;
    
    simWithstandDelayDA{n} = runCaDACombination(obj, DATStart, CaTStart, 1, 1, 1500, 30, 0.01, species{n}, effectors{n}, {});
    
    calcium{2} = simWithstandDelayDA{n}.Data{1}(2,:);
    time{2} = simWithstandDelayDA{n}.Time;
    ...
    for i = 1:length(DAdelay{n})
        ...
        simWithDAResults{n}{1,i} = thisSim{n}.Data{1}(1,:);
        calcium{i+2} = thisSim{n}.Data{1}(2,:);
        time{i+2} = thisSim{n}.Time;
    end
end

Nair_2016_original_Matlab_data{1}(:,1) = simWithoutDA{n}.Time;
Nair_2016_original_Matlab_data{1}(:,2) = simWithoutDA{n}.Data{1}(1,:);
Nair_2016_original_Matlab_data{2}(:,1) = simWithstandDelayDA{n}.Time;
Nair_2016_original_Matlab_data{2}(:,2) = simWithstandDelayDA{n}.Data{1}(1,:);
Nair_2016_original_Matlab_data{3}(:,1) = DAdelay{n};
Nair_2016_original_Matlab_data{3}(:,2) = activationAreaWithDAResults{n}/activationArea{n};

save("./Code for paper figures/Figure 3/Nair_2016_original_Matlab_data.mat",...
    "Nair_2016_original_Matlab_data")
save("./Code for paper figures/Figure 3/Nair_2016_calcium.mat","calcium","time")
...
% hold on
% for n = 3:4
% plot(simWithoutDA{1}.Time, simWithoutDA{1}.Data{1}(1,:)/max(simWithoutDA{1}.Data{1}(1,:)), '-.', 'LineWidth', 2, 'Color', mc(n))
% plot(simWithstandDelayDA{1}.Time, simWithstandDelayDA{n}.Data{1}(1,:)/max(simWithoutDA{1}.Data{1}(1,:)), 'LineWidth', 2, 'Color', mc(n))
% end
% set(gca,'FontSize',12, 'FontWeight', 'bold')
% xlabel('time (s)');
% ylabel('Substrate phosphorylation');
...
%     if sum(DAdelay{n}(i) == DAdelayplotpoint{n})
%         colorcounter = colorcounter +1;
%         legends{end+1} = ['\Deltat=' num2str(DAdelay{n}(i)) 's'];
%         plot(simWithoutDA{n}.Time, simWithDAResults{n}{i}/max(simWithoutDA{n}.Data{1}(1,:)), 'LineWidth', 2, 'Color', ocmap(2*colorcounter,:))
%     end
% end
% set(gca,'FontSize',12, 'FontWeight', 'bold')
% xlabel('time (s)');
