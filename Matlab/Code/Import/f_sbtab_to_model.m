...
sbtab.species = [sb.Compound.Name, sb.Compound.InitialValue, sb.Compound.IsConstant, sb.Compound.Unit, sb.Compound.Location];
% Combine default parameter related data into sbtab.defpar
sbtab.defpar = [sb.Parameter.Comment, sb.Parameter.Value_linspace, sb.Parameter.Unit];
...
for n = 1:size(sbtab.species,1)
    compartment_number_match = find_compartment_number(compobj, sb.Compound.Location{n});
    addspecies (compobj{compartment_number_match}, sb.Compound.Name{n},...
        sb.Compound.InitialValue{n}, 'InitialAmountUnits', sb.Compound.Unit{n});
end
...
for n = 1:size(sb.Experiments.ID,1)
    ...
    if isfield(sb.(experiment_field_name), "Time")
        Data(n).Experiment.t = transpose([sb.(experiment_field_name).Time{:}]);
    end
    ...
    for m = 1:size(sb.Compound.ID, 1)
        ...
        if isfield(sb.(experiment_input_field_name), "Input_Time_S" + (m - 1))
            n_input_time = n_input_time + 1;
            sbtab.datasets(n).input_time{1, n_input_time} = ...
                [sb.(experiment_input_field_name).("Input_Time_S" + (m - 1)){:}];
        end
    ...
    for m = 1:size(sb.Output.ID, 1)
        if isfield(sb.(experiment_field_name), "Y" + (m - 1))
            ...
            sbtab.datasets(n).output_location{n_output} = sb.Output.Location(m);
        end
    end
...
for reaction_idx = 1:num_reactions
    ...
    while contains(reaction, location + " " + location)
        reaction = strrep(reaction, location + " " + location," " + location);
    end
    ...
    set(reaction_object,'ReactionRate',sb.Reaction.KineticLaw{reaction_idx});
end
end
