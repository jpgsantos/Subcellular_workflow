...
if isempty(sbtab) || isempty(Data)
    if isfield(model_folders, 'model') && isfield(model_folders.model, ...
        ...
        error('Data model not found in model_folders. Please check the folder paths.')
    end
end
...
if isfield(sbtab, 'defpar')
    ...
    error('Default parameters not found in SBtab. Ensure SBtab is properly formatted.')
end
...
if isfield(settings, 'PLind') && isfield(settings, 'PLval')
    if settings.PLind > 0 && settings.PLind <= length(parameters) + 1
        ...
        error('PLind is out of bounds. It should be within the range of parameter indices.')
    end
end

% Update simulation parameters
sim_par = update_simulation_parameters(sim_par, parameters, settings, sbtab);
...
if ~isempty(settings.exprun) && isnumeric(settings.exprun)
    ...
    error('stg.exprun is invalid. Ensure exprun is a non-empty numeric array.')
end
...
for exp_indx = settings.exprun
    ...
    [species_start_amounts,success_eq(exp_indx), error_type] =...
        equilibrate_2(exp_indx, settings, sim_par, result, model_folders, sbtab, ...
        prev_success);
    ...
    for m = 1:count
        ...
        if settings.partest(settings.(key_1(k))(n, m), 1) > 0
            ...
            eval("sim_par(n) = sim_par(n) " + key_2(k) + ...
                " (10 ^ (parameters(settings.partest(settings.(key_1(k))(n, m), 1))));");
        else
            % Make the appropriate multiplications to get the
            % thermodynamically constrained parameter
            eval("sim_par(n) = sim_par(n) " + key_2(k) + ...
...
function [species_start_amounts,error_type] = ...
    equilibrate(n, settings, sim_par, result, model_folders, sbtab, species_start_amounts, success)
% Equilibrate the model
result = f_sim(n+settings.expn, settings, sim_par, species_start_amounts, result, model_folders, success);
error_type = "";
