function result = f_lsa(settings, model_folders)
...
number_samples = settings.lsa_samples;
range_from_best = settings.lsa_range_from_best;
...
B_star = ...
    settings.bestpa(1:parameter_n) + B_star .* ...
    (range_from_best * 2) - range_from_best;
...
result.B_star = B_star;
result.score_B_star = score_B_star;
result.x1 = x1_1;
result.x11 = x11_1;
result.x22 = x22_1;
result.P_matrix = P_matrix_1;
...
for n = number_samples: -1: 1
    for i = 1:parameter_n
             % disp("n: " + score_B_star(x11_1(n, i) + (n - 1) * ...
             %    (parameter_n + 1)).sd + ...
             %      " n+1: " + score_B_star(x11_1(n, i) + 1 + (n - 1) * ...
             %    (parameter_n + 1)).sd)
...
result.parameter_score = parameter_score;
for n = 1:parameter_n
    parameter_score_delta(:, n) = parameter_score(:, n) / delta_scaled(n);
end
result.parameter_score_delta = parameter_score_delta;
result.sum_parameter_score = sum(abs(parameter_score));
result.sum_parameter_score_delta = sum(abs(parameter_score_delta));
result.mean_parameter_score = sum(abs(parameter_score)) / number_samples;
result.mean_parameter_score_delta = ...
sum(abs(parameter_score_delta)) / number_samples;
result.sigma_parameter_score = ...
    sqrt((1 / (number_samples - 1)) * ...
    sum((parameter_score-sum(parameter_score)/number_samples) .^ 2));
result.sigma_parameter_score_delta =...
    sqrt((1 / (number_samples - 1)) * ...
    sum((parameter_score_delta - sum(parameter_score_delta) / ...
    number_samples) .^ 2));

% Store average and sigma deviations for each parameter
result.average_deviation = result.mean_parameter_score_delta(1, :);
result.sigma_deviation = result.sigma_parameter_score_delta;
end
...
x = num_samples * par_n_plus_1;

% Print progress information at each step
if mod(current_sample, ceil(x / par_n_plus_1)) == 0 &&...
        current_sample ~= x
    if (x-current_sample) / num_samples <=...
            par_n_plus_1 - 2

        dt = (datetime - last_time);
        remaining_time = seconds(dt);
        remaining_time = ...
            remaining_time * (x - current_sample) / num_samples;
