function rst = f_opt_fmincon(stg,script_folder)
...
if stg.mst
    parfor n = 1:stg.msts
        ...
        [x(n,:),fval(n),exitflag(n),output(n)] =...
            fmincon(@(x)f_sim_score(x,stg,script_folder),startpoint(n,:),...
    ...
    [x(1,:),fval(1),exitflag(1),output(1)] =...
        fmincon(@(x)f_sim_score(x,stg,script_folder),startpoint(1,:),...
        [],[],[],[],stg.lb,stg.ub,[],options);
end
