...
% - PL_iter: Current range of indices in the profile likelihood 
% calculation.
...
% - section: Part of the parameter space being optimized ('start' or
% 'end').
...
for PL_iter_current = PL_iter
    ...
    % Execute optimization for specific positions within the parameter
    % space
    while current_pos <= 4
        optimize = false;
        step = 0;
        if (current_pos == 2 && ~pass_pos(1)) || ...
            (current_pos == 4 && ~pass_pos(3))

            % Get the score for the current solution
            x_score = x{alg{2}}{pos_minus_1};
            [score,~,~] = ...
                f_sim_score(x_score, settings, model_folders, 0, 0);
        end
        ...
        if optimize
            ...
            % Execute the optimization method with updated settings and
            % current step
            [x, fval, Pval, prev_fval, offset] = ...
                run_optimization_method(x, fval, offset, temp_lb, temp_up, ...
                settings, model_folders, alg, PL_iter_current, ...
                current_pos, pos, pos_minus_1, 0);
        end
