...
plot_sections = {
    ...
    "(Bootstrapping Mean)"],...
    "outputNames.sd", "get_mean_values(rst.SiQ.sd,stg)";
    ...
    " calculated using the Score of each Experimental Output ",...
    " (Bootstrapping Mean)"],...
    "outputNames.sd", "get_mean_values(rst.SiTQ.sd,stg)";
    % Bootstrapping quartile mean of first order Sensitivity index for
    ...
    "(Bootstrapping Mean)"],...
    "outputNames.se", "get_mean_values(rst.SiQ.se,stg)";
    % Bootstrapping quartile mean of total order Sensitivity index for
    % score per Experiments
    "SiT se bm",...
    ["Total order Sensitivities",...
    " calculated using the Score of each Experiment",...
    "(Bootstrapping Mean)"],...
    "outputNames.se", "get_mean_values(rst.SiTQ.se,stg)";
    ...
    "(Bootstrapping Mean)"],...
    "outputNames.xfinal", "get_mean_values(rst.SiQ.xfinal,stg)"
    ...
    " calculated using the final value of each Experimental Output",...
    "(Bootstrapping Mean)"],...
    "outputNames.xfinal", "get_mean_values(rst.SiTQ.xfinal,stg)"
    };
...
function [plots1,plots2] =...
    f_generate_plot(rst,stg,outputNames,parNames2,name,major_title,...
...
function mean_values = get_mean_values(input_matrix,stg)
mean_values = transpose(reshape(mean(input_matrix(:,:,1:stg.parnum)),...
