...
function list_inputs = f_list_inputs(sbtab, list_inputs, exp_idx)
    ...
    % Extract input IDs from the current experiment and convert them to numbers more efficiently
    input_ids = cellfun(@(x) sscanf(x, 'S%d') + 1, sbtab.datasets(exp_idx).input);
...
for exp_idx = 1:num_experiments
    ...
    f_plot_input(stg, rst, sbtab, stg.exprun(exp_idx), all_list_inputs, colors);
...
function plot_input_legend(outer_layout, list_inputs, ...
    ...
    for n = 1:size(list_inputs, 2)
        ...
        input_Index = find(strcmp([all_list_inputs(2, :)], ...
            list_inputs{2, n}));
        % Create a scatter plot for the legend entry and store it in the array if it is valid
        ...
        if isvalid(scatter_handle)
            legend_items = [legend_items scatter_handle]; % Add valid handle to the array
        end
    end
    hold(ax, 'off');
...
plot_n = 0; % Counter for the number of plots created
fig_n = 0; % Counter for the number of figures created
layout = []; % Initialize layout as empty
plots = cell(1,2); % Initialize plots cell array
colors = generateRainbowGradient(length(stg.pat)); % Generate colors for plotting
...
for exp_idx = stg.exprun
    ...
    for out_idx = 1:size(sbtab.datasets(exp_idx).output,2)
        ...
        do_norm = ~isempty(sbtab.datasets(exp_idx).Normalize) + 1;

        for k = 1:do_norm

            % Generate a new figure after every 12 plots or if layout is empty
            if mod(plot_n,max_number_subplots_output) == 0 || isempty(layout)
                % Create new subplot layout
                [fig_n,layout,plots(fig_n,:),outer_layout] =...
                    f_get_subplot(n_outputs_exp_plus_norm,plot_n,fig_n,"Outputs",layout);
                % Set the figure name
            ...
            if k == 1
                % Plot the simulation outputs for the first normalization state
                [output_min,output_max] =...
                    plot_sim_outputs(stg,rst,sbtab,mmf,exp_idx,out_idx,~k,include_exp_n,colors);
                ylim([output_min output_max])
                % If no normalization, also plot the experimental data with SD
                if do_norm == 1
                    [min_data,max_data] = ...
                ...
                % Plot the experimental data and simulation outputs for normalized data
                [min_data,max_data] = ...
                    plot_data_and_data_SD(stg,rst,Data,exp_idx,out_idx);
                [output_min,output_max] =...
                    plot_sim_outputs(stg,rst,sbtab,mmf,exp_idx,out_idx,k,include_exp_n,colors);
            ...
            if mod(plot_n,max_number_subplots_output) == 0 || plot_n == n_outputs_exp_plus_norm
                % Create an invisible axis for the legend
                ax = axes(outer_layout, 'Visible', 'off');
                hold(ax, 'on');
                ...
                valid_outputs_plots_lgd = arrayfun(@(i) scatter(ax, NaN, NaN, 2, colors(i, :), "o", "filled", ...
                    "MarkerFaceAlpha", 1, "MarkerEdgeAlpha", 1, "DisplayName", "\theta_{" + stg.pat(i) + "}"), 1:length(stg.pat));
                hold(ax, 'off');
...
for exp_idx = stg.exprun
    ...
    for out_idx = 1:size(sbtab.datasets(exp_idx).output,2)
        ...
        [output_min, output_max] = plot_sim_outputs(stg, rst, sbtab, mmf, ...
            ...
            colors(different_input_number + 1:end, :));
        ylim([output_min output_max]);
...
% Helper function to create a new figure or reuse an existing one
...
% Determine if a new figure should be created or reused based on the number of subplots
...
outerLayout = tiledlayout(1, 3, 'Padding', "tight", 'TileSpacing', 'tight');

% Assign a title to the overall layout, enhancing readability and providing
% context for the plot.
title(outerLayout, name_long, 'FontSize', Major_title_FontSize, 'Fontweight', ...
    Major_title_Fontweight);

% Create an invisible axis for adding legends, which is essential for plots readability
ax = axes(outerLayout, 'Visible', 'off');

% Create layout for input subplots (single subplot for inputs)
% The innerLayout_input will hold a single input-related plot, with tight padding and spacing
innerLayout_input = tiledlayout(outerLayout, 1, 1, 'Padding', 'tight', 'TileSpacing', 'tight');

% Calculate row and column indices for output subplots
% Determine the number of remaining outputs for current figure
remaining_outputs = n_outputs_exp_plus_norm(exp_idx) - max_right_subplots * (fig_number_same_exp - 1);
% Use the minimum value between max_right_subplots and the number of remaining outputs
row_column_idx = min(max_right_subplots, remaining_outputs);

% Create layout for output subplots (multiple subplots)
% This layout is determined based on the calculated row and column indices to fit the outputs
innerLayout_output = tiledlayout(outerLayout, row_length(row_column_idx), column_length(row_column_idx), 'Padding', 'tight', 'TileSpacing', 'tight');
innerLayout_output.Layout.Tile = 2; % Position the output layout in the second tile of the figure
innerLayout_output.Layout.TileSpan = [1 2]; % Span the output layout across two tiles for better visibility and organization
end

function plot_in_out_legend(ax, sbtab, rst, stg, exp_idx, all_list_inputs, colors, different_input_number)
% Function to create a legend for input and output plots.
% This function adds legend entries for input species, experimental data, and model outputs.
...
for ipt_idx = 1:size(sbtab.datasets(exp_idx).input, 2)
    % Extract input species ID from the current experiment and convert it to a numerical value
    % The ID is extracted by removing the 'S' character and converting the remainder to a number.
    % Adding 1 to match MATLAB's 1-based indexing.
    input_species_ID = str2double(strrep(sbtab.datasets(exp_idx).input(ipt_idx), 'S', '')) + 1;
    
    % Get the input species name from the species table using the extracted ID
    input_species_name = convertCharsToStrings(sbtab.species{input_species_ID, 1});
    
    % Find the index of the input species in the list of all inputs to match the color
    % This ensures that the correct color is used for the legend entry.
    input_Index = find(contains([all_list_inputs(2, :)], input_species_name));
    
    % Create a scatter plot handle for the input species to be used in the legend
    % The plot is invisible (NaN coordinates), but it serves the purpose of creating a legend entry
...
% Create scatter plot handle for the experimental data to be used in the legend
% This handle is also invisible and serves to add an entry for experimental data
plot_data_lgd = scatter(ax, NaN, NaN, 2, 'k', "o", "filled", ...
    "MarkerFaceAlpha", 1, "MarkerEdgeAlpha", 1, "DisplayName", "data");

% Create a patch handle for data standard deviation (SD) to be used in the legend
% The patch is invisible and is used to represent the data SD in the legend
% The patch settings include no edge color and a semi-transparent face color to indicate variability.
plot_data_SD_lgd = patch(ax, NaN, NaN, zeros(1, 1), "DisplayName", "Data_{SD}", ...
    'EdgeColor', 'none', 'FaceColor', [0 0 0], 'FaceAlpha', 0.25, 'HandleVisibility', 'off');
...
for pa_idx = stg.pat
    ...
    % The plot is invisible (NaN coordinates), but it is used to create a legend entry for each parameter set
    valid_outputs_plots_lgd(1, current_plot) = scatter(ax, NaN, NaN, 2, ...
        colors(current_plot + different_input_number, :), "o", "filled", ...
        "MarkerFaceAlpha", 1, "MarkerEdgeAlpha", 1, "DisplayName", "\theta_{" + string(pa_idx) + "}");
end
...
plots_leg = [input_plot_lgd, plot_data_lgd, plot_data_SD_lgd, valid_outputs_plots_lgd];

% Create the legend with the specified properties
% Set properties like font size, weight, and orientation to maintain a consistent appearance
lgd = legend(plots_leg, 'FontSize', Legend_FontSize, 'Fontweight', Legend_Fontweight, ...
    "Orientation", "horizontal");

% Position the legend at the bottom of the layout to keep the figure uncluttered
...
function [input_plot] = f_plot_input(stg, rst, sbtab, exp_idx, all_list_imputs, colors)
% Function to plot the input data for a specific experiment.
% This function generates scatter plots of input data points for visualization.
%
% Inputs:
...
% - all_list_imputs: Cell array containing all input species across experiments.
...
% Preallocate input_plot array for efficiency, especially for large datasets.
num_inputs = size(sbtab.datasets(exp_idx).input, 2);
input_plot = NaN(1, num_inputs); % Preallocate with NaN to handle conditional plotting cases.
...
for ipt_idx = 1:num_inputs
    ...
    for pa_idx = stg.pat
        % Check if the simulation was successful for the current parameter set and experiment.
        if rst(pa_idx).simd{1, exp_idx} ~= 0
            % Extract the input species ID from the current experiment and convert it to a numerical value.
            % The ID is extracted by removing the 'S' character and converting the remainder to a number.
            input_species_str = strrep(sbtab.datasets(exp_idx).input(ipt_idx), 'S', '');
            if isempty(regexp(input_species_str, '^\d+$', 'once'))
                warning('Invalid input species ID format for experiment %d, input %d: %s', exp_idx, ipt_idx, sbtab.datasets(exp_idx).input(ipt_idx));
            ...
            if isnan(input_species_ID)
                warning('Invalid input species ID for experiment %d, input %d: %s', exp_idx, ipt_idx, sbtab.datasets(exp_idx).input(ipt_idx));
                continue; % Skip this input if the conversion fails.
            end
            
            % Get the name of the input species from the species table using the extracted ID.
            input_species_name = convertCharsToStrings(sbtab.species{input_species_ID, 1});
            
            % Find the index of the current input species in the list of all inputs.
            % This ensures that the correct color is used for the legend entry.
            input_Index = contains([all_list_imputs(2, :)], input_species_name);
            ...
            if log_10 == 0
                ...
                label_y = string(rst(pa_idx).simd{1, exp_idx}.DataInfo{input_species_ID, 1}.Units);
            else
                % Apply log10 transformation if specified.
                input_y = log10(rst(pa_idx).simd{1, exp_idx}.Data(1:end, input_species_ID));
                label_y = "Log10 " + string(rst(pa_idx).simd{1, exp_idx}.DataInfo{input_species_ID, 1}.Units);
            end
            
            % Create a scatter plot for the input species data points.
            % Only a subset of data points is plotted based on the 'in_plot_trim' setting to reduce clutter.
            input_plot(ipt_idx) = scatter(rst(pa_idx).simd{1, exp_idx}.Time(1:stg.in_plot_trim:end), ...
                input_y(1:stg.in_plot_trim:end), 2, "o", "filled", "MarkerFaceAlpha", 1, ...
                "MarkerEdgeAlpha", 1, "DisplayName", rst(pa_idx).simd{1, exp_idx}.DataNames{input_species_ID}, ...
                'MarkerFaceColor', colors(input_Index, :));

            % Label the y-axis with the correct unit and apply pre-defined font settings.
            ylabel(label_y, 'FontSize', Axis_FontSize, 'Fontweight', Axis_Fontweight);

            adjust_y_ticks()

            % Set y-axis limit to start from zero and extend to positive infinity.
            ylim([0 inf]);
...
% Get the current y-axis tick values to format precision if needed.
            ...
            % If there are too many decimal places, limit the precision to two decimal places.
            if max([countDecimals{:}]) > 2
                ytickformat('%.2f'); % Limit y-axis tick labels to two decimal places for better readability.
            end
end
