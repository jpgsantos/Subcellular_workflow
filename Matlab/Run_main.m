function [settings, sb, results, model_folders, plots] = ...
...
if ~contains(strrep(path, "\", "/"), ...
        extractAfter(Matlab_main_folder, ...
        strlength(Matlab_main_folder) - 1))
...
% Runs the Analysis chosen by the user input
     [results, settings] = ...
         f_analysis(settings, settings.analysis, model_folders, ...
         analysis_options, results);
