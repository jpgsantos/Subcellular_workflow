...
for j = 1:size(sbtab.datasets(n).output,2)
    ...
    ylabel('nmol/l','FontSize', 8,'Fontweight','bold')
...
layout4 = tiledlayout(layout,1,2,'TileSpacing','none');
...
% end
    ...
    function f_plot_in_out_left(rst,stg,sbtab,helper,reuse)
        if reuse
            ...
            layout = tiledlayout(5,1,'Padding','none','TileSpacing','compact');
            ...
            figure('WindowStyle', 'docked','Name', 'Paper Figure 3',...
                'NumberTitle', 'off');
            layout = tiledlayout(5,1,'Padding','none','TileSpacing','normal');
            
        end
        
        layout1 = tiledlayout(layout,1,5,'TileSpacing','none');
        layout1.Layout.Tile = 1;
        layout1.Layout.TileSpan = [3,1];
        layout3 = tiledlayout(layout1,1,1,'TileSpacing','none');
        layout3.Layout
        ...
        title("Ca + Da (\Deltat=1s)",'FontSize', 8,'Fontweight','bold')
        txt = text(-2.5,5750,'A');
%         txt.FontSize = 8;
        txt.FontWeight = 'bold';
        txt = text(22.5,5750,'B');
%         txt.FontSize = 8;
        txt.FontWeight = 'bold';
        hold off
        
        if size(sbtab.datasets(n).output,2) == 1
            layout2 = tiledlayout(layout1,1,1,'TileSpacing','compact');
        elseif size(sbtab.datasets(n).output,2) == 2
            layout2 = tiledlayout(layout1,1,2,'TileSpacing','compact');
        elseif size(sbtab.datasets(n).output,2) > 2 &&...
                size(sbtab.datasets(n).output,2) <= 4
            layout2 = tiledlayout(layout1,2,2,'TileSpacing','none');
        end
...
if f_check_minimum_version(9,8)
    layout.Units = 'inches';
    layout.OuterPosition = [0 0 6.5 4];
    
    pause(5)
    
    exportgraphics(layout,...
        'Reproduce paper figure 3/Figure 3 alt2.png',...
        'Resolution',600)
%     
